...
### 有料note（販売）について — 価格・有料ラインまで自動

`sale.enabled: true` にすると、次までを自動で行います:

1. AIが「無料導入パート」と「有料本編」を書き分ける
2. エディタで無料パートを入力 → **本文中に有料ラインを自動挿入** → 有料パートを入力
3. 公開設定で **有料を選択し、`sale.price` の価格を自動入力**
4. `publish.mode: publish` なら公開まで実行（`draft` なら設定済みの状態で停止）

> ⚠️ **有料設定のUIは特に変わりやすい部分**です。うまくいかない場合は
> `shots/` に保存される失敗時スクリーンショットを見て、`publisher.py` の
> `_insert_paywall_line()` / `_configure_paid_and_publish()` のセレクタや
> ボタン文言を調整してください。安全のため、まず `publish.mode: draft` で
> 有料ラインと価格が正しく入るかを確認してから公開に切り替えるのを推奨します。
