...
def build_caption(name, price, genre_name):
    ...
    # ジャンルごとに雰囲気の合う「ひとこと」を選ぶ
    hooks = {
        "総合":                 "今みんなが買ってる話題のアイテム✨",
        "レディースファッション": "気分が上がる今どきコーデに🎀",
        "メンズファッション":     "さりげなく差がつく大人アイテム👔",
        "美容・コスメ・香水":     "自分にごほうび、毎日のケアに💄",
        "食品":                 "おうち時間がちょっと幸せになる一品🍽️",
        "インテリア・寝具・収納":  "お部屋がもっと好きになる♡",
    }
    hook = hooks.get(genre_name, "見つけた瞬間ポチりたくなるやつ🛒")

    # ハッシュタグ（楽天ROOMは #タグ で見つけてもらいやすくなる）
    tags = ["#楽天ROOM", "#楽天お買い物マラソン", "#楽天room初心者",
            "#おすすめ商品", "#ポチったもの"]
    genre_tag = {
        "レディースファッション": "#プチプラコーデ",
        "メンズファッション":     "#メンズファッション",
        "美容・コスメ・香水":     "#コスメ好きさんと繋がりたい",
        "食品":                 "#お取り寄せグルメ",
        "インテリア・寝具・収納":  "#お部屋作り",
    }.get(genre_name)
    if genre_tag:
...
def demo_posts():
    ...
    samples = [
        ("総合", "【楽天1位】ふわふわ タオル 5枚セット 今治産", 2480,
