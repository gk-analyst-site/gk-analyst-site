...
def build_caption(name, price, genre_name):
    ...
    generic = [
        "暮らしの「あってよかった」、そっとおすそ分け🍀",
        "見つけた瞬間、ちょっとうれしくなったやつ✨",
        "生活がちょっと軽くなるかも🛍️",
    ]
    variations = hooks.get(genre_name, generic)
    # 商品名から決める（同じ商品はいつも同じ、違う商品は違う1文目に）
    idx = sum(ord(c) for c in name) % len(variations)
    hook = variations[idx]
